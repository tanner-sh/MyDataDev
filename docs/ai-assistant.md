...
**系统提示里一直写着「找不到时明确询问用户」，但那句话没有出口。** 反问和答案一样是一段正文，
界面只能当普通回答渲染：用户得把问题读出来，再自己组织语言打一遍回答 —— 而重新组织的措辞
很可能不是模型预期的说法，于是又要多问一轮。

更要命的是打分。评测集里每条用例的正确答案都是一条 SQL，**猜一个总比问一句得分高**，
所以这套机制实际上在教模型别问。这一期把出口和打分一起补上。

- **反问做成工具 `ask_user`**（`AiClarify`），不是约定一段 JSON 正文：函数调用是模型本来就在用的
  通道，参数由 provider 保证是结构化的；让它在正文里输出 JSON，则每一次格式抖动都要在这边
  写解析补丁。
- **选项是可选的**。「你指的是哪张客户表」给得出选项，「这个订单号是多少」给不出 —— 强行要求
  只会让模型编造几个。最多五个，再多用户就不是在选而是在读清单。
- **`ask_user` 由 `AiSqlAgentService` 拦下，不进 `AiSchemaTools`** —— 那一层是只读元数据工具，
  反问是控制流。但**它同样要有工具结果**：少一条，下一轮的历史里就留下一个没有结果的工具调用，
  两家协议都会直接报错，而那要到用户回答之后才炸。所以先把整批工具结果补齐，再收尾。
- **审计里 `outcome=clarified`**，与 success / error / cancelled 并列。这个数值得单独盯：一路是 0
  说明模型还是在猜，太高则说明它在拿工具能回答的问题去麻烦用户。
- 反问是一条正常的可见消息，**结构化的问题与选项存进会话**（`AiChatMessageResponse.question`），
  刷新页面后按钮还在。可见正文只存问题本身，选项走结构化字段 —— 排版是界面的事，两边不必
  各写一份。界面上只有对话里**最后一条**消息的选项才画成按钮：往上翻能看到几轮前问过什么，
  但那些问题早就被后面的回答接上了，再摆一排按钮，点下去等于把答过的问题重答一遍。
- **评测集加了三条反问用例**（`ambiguous-user`、`ambiguous-amount`、`ambiguous-active`），
  打分反过来：问了才算通过，给出 SQL 就是「该问却直接猜了」。判断标准写在提示里也写在用例里 ——
  这个答案会不会改变最终的 SQL；能靠工具查清楚的一律自己查。
- 用例集因此从 10 条变成 13 条，**总通过率和之前的基线不再直接可比**，比的时候看逐条结果。
...
**主要校验「这条 SQL 命中了哪些表」，不比对 SQL 文本。**（M15 起还有三条反问用例，
正确答案是问一句而不是给 SQL，见下。） 同一个需求有无数种写法，比对文本量的是模型的风格；而找没找对表恰恰是这套 Agent 存在的理由 —— 表错了，后面写得再漂亮都是错的。判「通过」要同时满足四条：期望的表一张不少、禁用的干扰表一张没有、`expectedTokens` 里决定口径的字段都用上了、SQL 通过目标库编译校验。多带一张表只记录不扣分。
