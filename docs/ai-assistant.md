...
2026-09-04，`deepseek-v4-flash`（DeepSeek 兼容协议）：

| 通过率 | 平均轮次 | 平均工具调用 | 平均输入 token | 平均输出 token | 平均缓存读 | 平均耗时 |
| --- | --- | --- | --- | --- | --- | --- |
| 10/10 | 5.1 | 6.1 | 17221 | 1809 | 14669 | 13.9 秒 |

三点读法：

- **缓存读占了输入的 85%**（兼容协议的 `prompt_tokens` 含缓存，所以每次真正新增的只有约 2.5k）。这说明多轮之间的前缀确实是稳定增长的，没有被每次都变的内容写脏 —— 系统提示里不放时间戳这条约定是有效的。
- **平均 6.1 次工具调用**偏高：模型要摸索六次才敢下笔。这是目前最值得优化的一项，也是判断「`search_schema` 的排序够不够准」最直接的指标。
- 同一套用例两次跑出来的工具调用次数是 6.5 和 6.1，**单次结果有噪声**，比较时看趋势不看小数点。
