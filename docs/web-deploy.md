...
**迁移发生在密钥校验之前**：新版本即使因为没有接管旧密钥而拒绝启动，元数据库也已经迁到新 schema 了。所以第 2 步的备份不是可选项 —— 一次失败的启动同样会让旧版本 JAR 打不开这个数据目录（Flyway 会报「已应用的迁移在本地找不到」）。
...
0.5.0（schema v13）升级到 0.6.0（v20）会依次应用 V14–V20：AI 配置与用量、业务词典、定时导出任务与运行记录、结构快照。它们只作用在本版本自己新建的表上（唯一的两条 `ALTER` 是 V17 给 V14 刚建的 `ai_settings` 加两列），不触碰升级前已有的连接、加密密码、SQL 历史与审计数据 —— 发布前用一套真实的 0.5.0 数据目录验证过：接管旧密钥后启动，原有账号、连接与加密密码都还在。

0.6.0 升级到 0.7.0 没有新增迁移（schema 仍是 v20），直接替换发行包即可。
