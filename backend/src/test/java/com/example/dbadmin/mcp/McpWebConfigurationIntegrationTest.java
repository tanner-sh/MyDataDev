...
class McpWebConfigurationIntegrationTest {
    ...
    @Test
    void configuresEnablesRotatesAndDisablesMcpWithoutRestart() throws Exception {
        ...
        mvc.perform(put("/api/mcp/status")
                ...
                .andExpect(jsonPath("$.message").value(org.hamcrest.Matchers.containsString("至少需要一个")));
        ...
        initialize(firstCredential).andExpect(status().isOk());
    ...
    private org.springframework.test.web.servlet.ResultActions initialize(String credential) throws Exception {
        org.springframework.test.web.servlet.ResultActions action = mvc.perform(post("/mcp")
