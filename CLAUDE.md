...
# --serve 会播一条 H2 连接和 240 行数据，走一遍 选连接→打开表→翻页→改一格→提交→导出
...
- **`core/DatabaseDialect` + `DialectRegistry`**：所有数据库差异（分页 SQL、EXPLAIN、DDL 生成、标识符引用、schema/catalog 语义、对象能力矩阵）都收敛到方言接口。`DialectRegistry` 按声明顺序做 `supports(dbType, jdbcUrl)` 首次匹配，顺序有意义（OceanBase 在 Oracle/MySQL 之前，`DefaultDialect` 兜底）。新增数据库支持 = 新增一个 dialect 并插入到正确位置，而不是在 service 里加 if。切换命名空间同样归方言管：借出时记下、归还时还原都走 `currentNamespace` / `activateNamespace`（见 `NamespaceScopedConnection`），**不要直接调 JDBC 的 `getSchema` / `setSchema`** —— OceanBase 的 Oracle 模式在默认驱动配置下根本没实现它们，抛的是 `AbstractMethodError`，会越过所有按 `Exception` 写的收尾，把借出的连接漏在池外。
...
- **`service/ScheduledQueryService` + `ScheduledQueryScheduler`**：定时导出。cron、时区与「下次执行」的判定与 `BackupScheduler` 同源，执行则复用 `ExportService.prepare` 那条路 —— 只读作用域、行数与字节上限、审计与 SQL 历史都不重写。**只导出，不执行写操作**：`ExportService.requireSingleQuery` 是这条规则的唯一定义，保存任务时就调一次，不要留到调度线程半夜发现。生产连接的确认在创建任务时完成（`production_confirmed`），运行时凭那次确认放行；定时任务没有交互确认的机会，而完全跳过确认等于给生产库开了一个无人值守的出口。
...
- **表数据导出走浏览那条查询**（`DataEditService.exportQuery` + `ExportService.prepareGenerated`）：服务端把浏览用的 SQL 去掉分页重跑，因此导出的内容与屏幕上的筛选、排序完全一致。**不要在前端另拼一条 SELECT** —— 筛选算子的语义、标识符引用和类型绑定都在 `DataEditService` 里，复制一份就会出现「界面上 12 行、导出 40 行」这种对不上的情况。导出仍要生产确认：浏览是把数据显示在屏幕上，导出是把它带出系统。
- **`service/SchemaSnapshotService` + `SchemaSnapshotScheduler`（Flyway V20）**：结构快照与漂移追踪。「结构对比」回答的是「这两个库现在一不一样」，回答不了「这个字段是什么时候加的」，而生产库上后一个问题往往更急。快照存的就是一份按表名排序的 `ObjectDetail` 列表（JSON），漂移把两份反序列化后交给 `SchemaComparison.compare` —— **与结构对比同一套判定，不另写一份**。四条约定：**结构没变时不新增行，只推进 `last_seen_at`**（时间线上留下的应该是「结构变了」的时刻，不是「我们检查过」的时刻；每小时采一次都增行的话，一周后要在 168 条一模一样的记录里找那一次真正的变化）；**`structureVersion` 必须在存快照前抹掉**，它是一次读取的指纹而不是结构的一部分，留着会让两份相同的结构算出不同校验和，于是每次采集都多出一条假的「变了」；**刻意不生成「改回去」的 DDL** —— 快照是记录不是期望状态，一个字段可能是上周有人有意加的，照旧快照删掉它才是真正的事故，要脚本请走结构对比拿真正的基准环境作源端；**审计线索只是线索**，`SchemaDriftResponse.auditEvents` 只包含经由本工具的结构变更，为空的正确读法是「不是从这里改的」而不是「没人改过」，这句话由 `schemaSnapshot.ts` 的 `driftAuditNotice` 负责说清楚。单张表读不出来时快照不完整，必须写进 warnings —— 否则下次拿它去比会把这张表报成「被删了」，一个凭空捏造的漂移。cron/时区判定与 `BackupScheduler`、`ScheduledQueryScheduler` 同源；快照按外键挂在连接上（删连接一并清理），保留份数有硬上限，因为存的是整份结构。
- **例程的编译错误与输出（`DatabaseDialect.compilationErrorsSql` / `routineOutputEnableSql` / `routineOutputFetchCall`）**：参数化调用（IN/OUT/INOUT、返回值、多结果集）本来就在 `SchemaObjectService.invoke` 里，缺的是另外两件。**「语句没抛异常」不等于「对象是好的」** —— Oracle 的 `CREATE OR REPLACE` 语法错了也返回成功，对象以 INVALID 状态留在库里，不查 `ALL_ERRORS` 的话界面给出的是一句干净的「已创建」，直到某天有人调用它才发现是坏的。字典表查询按 owner / 对象名 / 对象类型三个参数绑定，**名字必须大小写不敏感匹配**（Oracle 把未加引号的标识符折成大写存进字典，而表单里填的是小写，按原样比对永远返回空，这个功能就等于没做）。查不到就当没有编译错误：这是补充诊断，不该让一次已经执行完的 DDL 变成失败；查到了则响应里带 `compilationErrors`，界面不关闭编辑器（对象确实建出来了，但接下来要做的是改源码重提）。**例程输出有两个来源**：JDBC 的 SQLWarning 链（PostgreSQL 的 `RAISE NOTICE`、MySQL 的警告）和 Oracle 的 `DBMS_OUTPUT` 缓冲 —— 后者不显式 ENABLE + 逐行 GET_LINE 就永远看不到，而 PL/SQL 里排查问题基本全靠它。调用前要 `connection.clearWarnings()`，池化连接上旧警告会被当成这次调用的输出。整段吞异常：读不回消息不该把一次已经成功的调用改判成失败。
- **锁与阻塞分析（`DatabaseDialect.blockingSessionsSql` + 前端 `databaseSessions.ts` 的 `analyzeBlocking`）**：会话列表能让人杀掉一个会话，却说不出该杀哪一个 —— 中间这一步才是排查锁等待时真正要的东西。方言返回一条查「谁在等谁」的语句（MySQL 用 `sys.innodb_lock_waits`、PostgreSQL 用 `pg_blocking_pids`、Oracle 用 `V$SESSION.BLOCKING_SESSION`、MariaDB 用 `information_schema.INNODB_LOCK_WAITS`），列标签固定为 blocked_session_id / blocking_session_id / wait_object / wait_seconds / blocked_sql，**两个会话号必须与 `activeSessionsSql` 的 session_id 同一套编号**，否则边和会话拼不到一起，界面上只剩一堆孤立节点（`DatabaseSessionCapabilityTest` 按方言清单守着这两条）。服务端只取直接阻塞边，**成链是纯图运算，放在前端的 `analyzeBlocking` 里做**，因此能被单测覆盖；那里必须挡住互相等待的环（递归会无限展开），环上的会话单独报出来 —— 那已经不是「谁堵了谁」而是死锁。每个节点带下游被堵总数，根节点那个数就是「终止它能放开几个会话」。阻塞关系读失败不牵连会话列表：`blockingMessage` 与 `message` 分开，「会话读得到、阻塞读不到」（缺 PROCESS 权限或版本没有那张视图）是很常见的组合。
- **`service/DataSearchService`**：全库数据检索（在一个 Schema 的所有表里找一个值）。`ObjectSearchService` 搜的是对象名，这条搜的是值本身。三条约定：**比较语义与结果表格的筛选完全同源**（`SqlResultPushdown.textExpression` + `like`，都提成了包内可见的静态方法）—— 「搜到了」和「筛出来」必须是同一件事，否则用户点进那张表会发现筛不出刚看到的那一行；**扫描的三条上限（表数、每表取样行数、总时间预算）任何一条触顶都要写进 `stopReason` 并把 `complete` 置 false**，把「扫了 40 张表就没时间了」报告成「全库只有这些」会让人放心地得出错误结论，比没有这个功能更糟；**二进制与大对象列不参与检索**，跳过了哪些写进 warnings（与数据对比同源）。视图不扫 —— 数据来自底下的表，扫了只会把同一行报两遍。每张命中表回传一条可直接执行的查询（值用 `scriptLiteral` 回填），点开就是 SQL 工作台那条路，生产确认与审计照旧。生产连接上检索本身也要确认：几百张表各跑一条整表扫描是一次实打实的负载事件，和「打开一张表看看」不是一回事。
- **`service/DataTransferService` + `ResultSetRowSource`**：跨连接数据传输（A 库的表或查询结果 → B 库的表）。**没有新建任何后台任务机制** —— 读侧照 `ExportService` 的规矩（只读作用域、流式游标、生产确认、审计与 SQL 历史），写侧把 `ResultSetRowSource` 交给 `DataImportService.convert`，产出的脚本注册成一个普通的 SQL 文件任务，于是进度、取消、每连接并发闸门、未限定范围写确认与目标端的生产确认全部沿用现成那一套。三条约定：**不经 CSV 中转**（`ResultSet.wasNull()` 分得清空串与 NULL，往返一趟就分不清了 —— 这是 `ImportRowSource.distinguishesNull()` 存在的全部理由）；**二进制列在构造 `ResultSetRowSource` 时就拒绝并点名**，没有可移植的脚本字面量写法，写进去就是各库解释不同的值；**布尔一律写成 `'1'`/`'0'`**，这是 MySQL(TINYINT)、PostgreSQL(boolean)、SQL Server(BIT)、Oracle(NUMBER) 四家都接受的唯一写法，`'TRUE'` 在 MySQL 上直接报错。列按名字匹配目标表，改名的手段是在源 SQL 里写 `AS`。源端要单独记一条 `DATA_TRANSFER_READ` 审计：`uploadScript` 记的是目标连接那一半，少了这条，「这批数据什么时候被谁搬走的」在按源连接筛选时永远查不出来。不套导出那个一万行上限（那是为下载定的），规模边界是脚本字节上限。
- **导入的冲突策略收敛在方言里**（`DatabaseDialect.importConflictStyle`）：MySQL 改的是语句前缀（`INSERT IGNORE`），PostgreSQL 加的是后缀（`ON CONFLICT`），Oracle 要整条换成 `MERGE` —— 所以接口只声明「前缀 + 后缀」这个形状，写法交给方言，**不支持就返回 null**，由上层给出一句说得清楚的错误，而不是生成一条跑不通的脚本。默认仍是「直接插入」（撞了整批失败）：跳过会悄悄少写行、更新会覆盖库里已有的值，两件事都必须由用户明确选择。每条语句带几行同样归方言管（`DatabaseDialect.supportsMultiRowValues`）：Oracle 23 之前不认多行 `VALUES`（ORA-00933），SQL 逻辑备份与导入（含跨库传输）都按它逐行生成 —— 不要在 service 里按 dbType 再写一份清单，备份原来那份写死的清单就是导入漏掉 Oracle 的原因。
