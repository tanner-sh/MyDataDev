...
- **导入的冲突策略收敛在方言里**（`DatabaseDialect.importConflictStyle`）：MySQL 改的是语句前缀（`INSERT IGNORE`），PostgreSQL 加的是后缀（`ON CONFLICT`），Oracle 要整条换成 `MERGE` —— 所以接口只声明「前缀 + 后缀」这个形状，写法交给方言，**不支持就返回 null**，由上层给出一句说得清楚的错误，而不是生成一条跑不通的脚本。默认仍是「直接插入」（撞了整批失败）：跳过会悄悄少写行、更新会覆盖库里已有的值，两件事都必须由用户明确选择。
