...
| **MCP Server** | 通过 Agent API Key、连接白名单和生产环境授权，为 AI 客户端提供元数据浏览、表数据浏览、查询和执行计划。访问档位按连接授予（只读 / 数据读写 / 完全），默认只读；写操作与界面共用同一条执行路径，只读连接、生产确认、未限定范围写确认与审计一个都不少。 |
| **Web 与桌面双模式** | Web 版内置多用户、管理员/操作员角色、服务端会话与通用 OIDC 单点登录；桌面版内置前端、后端和 Java Runtime。 |
...
MyDataDev 当前版本为 `0.4.0`，主要面向本机与可信私有网络使用。项目仍在持续完善中；建议在重要数据库上先使用只读账号和测试环境验证，再逐步启用写入、备份与恢复能力。
