...
const ACTIONS: Readonly<Record<string, ActionMeta>> = {
  ...
  SCHEMA_SNAPSHOT_CAPTURE: { label: '采集结构快照', category: 'data' },
  SCHEMA_SNAPSHOT_UNCHANGED: { label: '结构快照无变化', category: 'data' },
  SCHEMA_SNAPSHOT_TARGET_SAVE: { label: '保存结构快照采集任务', category: 'data' },
  SCHEMA_SNAPSHOT_TARGET_DELETE: { label: '删除结构快照采集任务', category: 'data' },
  SCHEMA_DRIFT: { label: '结构漂移对比', category: 'data' },
  DATA_SEARCH: { label: '全库数据检索', category: 'data' },
  ...
  AI_EXPLAIN_INSIGHT: { label: 'AI 解读执行计划', category: 'ai' },
  AI_INTERPRET_RESULT: { label: 'AI 解读查询结果', category: 'ai', dangerous: true },
  AI_DOCUMENT_SCHEMA: { label: 'AI 生成数据字典', category: 'ai' },
  AI_REVIEW_SCRIPT: { label: 'AI 解读同步脚本风险', category: 'ai' },
